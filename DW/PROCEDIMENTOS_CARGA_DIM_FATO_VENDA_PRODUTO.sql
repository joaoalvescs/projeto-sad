...
EXEC SP_FATO_VENDA_PRODUTO '20190101'
					...
					IF(EXISTS(SELECT 1 FROM DIM_ENDERECO_CLIENTE WHERE COD_ENDERECO = @cod_endereco))
						BEGIN
							IF(EXISTS(SELECT 1 FROM DIM_ENDERECO_CLIENTE WHERE (RUA <> @rua OR NUMERO <> @numero OR CIDADE <> @cidade OR
																				BAIRRO <> @bairro OR ESTADO <> @estado OR CEP <> @cep) AND COD_ENDERECO = @cod_endereco))
							BEGIN
...
EXEC SP_DIM_ENDERECO_CLIENTE '20190101'
...
EXEC SP_DIM_PLATAFORMA '20190101'
...
EXEC SP_DIM_PAGAMENTO '20190101'
SELECT * FROM DIM_PAGAMENTO
